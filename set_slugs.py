...
# Explicit mapping: relative path -> ASCII slug
slug_map = {
...
for dirpath, dirnames, filenames in os.walk(docs_root):
    for fn in filenames:
        ...
        rel_path = os.path.relpath(fpath, docs_root)
        rel_path_no_ext = os.path.splitext(rel_path)[0].replace(os.sep, '/')

        if rel_path_no_ext not in slug_map:
            print(f"WARN: no slug for {rel_path_no_ext}")
            continue

        slug = slug_map[rel_path_no_ext]

        with open(fpath, 'r', encoding='utf-8') as f:
            content = f.read()

        if 'slug:' in content[:300]:
            content = re.sub(r'^slug:.*$', f'slug: {slug}', content, count=1, flags=re.MULTILINE)
        else:
            lines = content.split('\n')
            new_lines = []
            for line in lines:
