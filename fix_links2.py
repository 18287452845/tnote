...
for dirpath, dirnames, filenames in os.walk(docs_root):
    for fn in filenames:
        ...
        def replace_md_link(m):
            ...
            link_path = m.group(2)
            # Remove .md extension
            clean_path = link_path.rstrip('.md')
            # Decode URL encoding
            clean_path = unquote(clean_path)
            # Remove hash suffix if present
            clean_path = re.sub(r'\s+[0-9a-f]{32}$', '', clean_path).strip()

            # Try to match against slug_map
            # The link could be relative from current file
            rel_dir = os.path.relpath(dirpath, docs_root).replace(os.sep, '/')
            # Try both absolute and relative interpretations
            candidates = [
                clean_path,  # relative as-is
                os.path.normpath(os.path.join(rel_dir, clean_path)).replace(os.sep, '/'),
            ]

            for candidate in candidates:
                if candidate in slug_map:
                    fix_count += 1
                    return f'[{link_text}]({slug_map[candidate]})'
